-- Copiado de database/migrations/001_initial_schema.sql
-- Migração inicial para Supabase CLI
...
CREATE INDEX IF NOT EXISTS idx_admin_tenants_user ON admin_tenants(user_id);
CREATE INDEX IF NOT EXISTS idx_admin_tenants_tenant ON admin_tenants(tenant_id);
...
CREATE INDEX IF NOT EXISTS idx_form_fields_form ON form_fields(form_definition_id);
CREATE INDEX IF NOT EXISTS idx_form_fields_order ON form_fields(form_definition_id, order_index);
...
CREATE INDEX IF NOT EXISTS idx_submissions_form ON submissions(form_definition_id);
CREATE INDEX IF NOT EXISTS idx_submissions_status ON submissions(payment_status);
CREATE INDEX IF NOT EXISTS idx_submissions_external_id ON submissions(payment_external_id);
...
CREATE INDEX IF NOT EXISTS idx_payment_events_submission ON payment_events(submission_id);
...
CREATE INDEX IF NOT EXISTS idx_message_logs_submission ON message_logs(submission_id);
...
CREATE INDEX IF NOT EXISTS idx_schedule_jobs_tenant ON schedule_jobs(tenant_id);
CREATE INDEX IF NOT EXISTS idx_schedule_jobs_scheduled ON schedule_jobs(scheduled_for);
...
CREATE INDEX IF NOT EXISTS idx_outbound_webhook_configs_tenant ON outbound_webhook_configs(tenant_id);
...
CREATE INDEX IF NOT EXISTS idx_enrollment_logs_tenant ON enrollment_logs(tenant_id);
CREATE INDEX IF NOT EXISTS idx_enrollment_logs_submission ON enrollment_logs(submission_id);
CREATE INDEX IF NOT EXISTS idx_enrollment_logs_status ON enrollment_logs(status);
...
CREATE INDEX IF NOT EXISTS idx_mercadopago_configs_tenant ON mercadopago_configs(tenant_id);
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_whatsapp_configs_updated_at') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_outbound_webhook_configs_updated_at') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_mercadopago_configs_updated_at') THEN
...
-- Helper function to get current user's role (moved to public schema)
CREATE OR REPLACE FUNCTION public.user_role()
RETURNS user_role AS $$
    SELECT role FROM users WHERE auth_user_id = auth.uid();
$$ LANGUAGE sql SECURITY DEFINER;

-- Helper function to check if user is admin of a tenant (moved to public schema)
CREATE OR REPLACE FUNCTION public.is_admin_of_tenant(tenant_uuid UUID)
RETURNS BOOLEAN AS $$
    SELECT EXISTS (
...
CREATE POLICY "Tenant-based access for form_definitions"
    ...
    USING (
        public.user_role() = 'superadmin' OR
        (public.user_role() = 'admin' AND public.is_admin_of_tenant(tenant_id))
    );
